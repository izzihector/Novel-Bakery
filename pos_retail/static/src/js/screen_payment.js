...
odoo.define('pos_retail.screen_payment', function (require) {
    ...
    screens.PaymentScreenWidget.include({
        ...
        renderElement: function () {
            ...
            this.$('.paid_full').click(function () {
                ...
                var register = _.find(self.pos.cashregisters, function (register) {
                    var order_currency = order.currency;
                    if (order_currency) {
                        var pos_config_currency_id = self.pos.config.currency_id[0];
                        var config_currency = self.pos.currency_by_id[pos_config_currency_id];
                        if (config_currency && config_currency.id != order_currency.id) {
                            return register['journal']['pos_method_type'] == 'default' && register['journal']['currency_id'][0] == order_currency['id'];
                        }
                    } else {
                ...
                if (register) {
                    ...
                    self.pos.gui.show_popup('dialog', {
                        title: 'Warning',
                        body: 'Can not find cash register the same currency with selected currency'
                    })
                }
            });
        ...
        render_paymentmethods: function() {
            var self = this;
            console.log("current pos",this.pos.cashregisters);
            this.pos.cashregisters =   this.pos.cashregisters.sort(function (a, b) {
                            return a.journal.sequence - b.journal.sequence;
                        });
            console.log("pos cash registers",this.pos.cashregisters);
