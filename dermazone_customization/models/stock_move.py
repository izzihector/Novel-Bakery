...
class InheritMoveLines(models.Model):
    ...
    expiry_date = fields.Datetime(related='lot_id.use_date')
    expiry_date_input = fields.Datetime('Expiry Date')
    production_date_input = fields.Datetime('Production Date')
                            ...
                            if ml.lot_name and not ml.lot_id:
                                lot = self.env['stock.production.lot'].create(
                                    {'name': ml.lot_name, 'product_id': ml.product_id.id,
                                     'use_date':ml.expiry_date_input,'production_date':ml.production_date_input}
                                )
...
class StockMove(models.Model):
    ...
    def _update_reserved_quantity(self, need, available_quantity,
                                  location_id, lot_number=None,
                                  package_id=None, owner_id=None,
                                  strict=True):
        if self.sale_line_id.lot_number:
            lot_id = self.sale_line_id.lot_number
        return super()._update_reserved_quantity(
            need, available_quantity, location_id, lot_id=lot_number,
            package_id=package_id, owner_id=owner_id, strict=strict)

    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        vals = super()._prepare_move_line_vals(
            quantity=quantity, reserved_quant=reserved_quant)
        lot = self.sale_line_id.lot_number
