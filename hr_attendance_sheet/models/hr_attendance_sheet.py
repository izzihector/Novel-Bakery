...
class attendance_sheet(models.Model):
    ...
    def calculate_att_data(self):
        ...
        for att_sheet in self:
            ...
            for line in att_sheet.att_sheet_line_ids:
                ...
                if line.diff_time > 0 and line.status != "ab":
                    diff += line.diff_time
                    no_diff += 1
                if line.status == "ab":
                    no_absence += 1
                    absence_hours += line.act_diff_time
                if line.late_in > 0:
                    late += line.late_in
                    no_late += 1
                            ...
                            if att_work_intervals:
                                if len(att_work_intervals) > 1:
                                    ...
                                    for att_work_interval in att_work_intervals:
                                        ...
                                        print ('float worked hours is', float_worked_hours)
    ...
    def action_payslip(self):
        ...
        if not payslip_id:
            payslip_id=self.update_payslip()[0]
        return {
    ...
    def update_payslip(self):
        ...
        for att_sheet in self:
            if payslips.employee_id == self.employee_id.id and payslips.state == 'draft':
                continue
            from_date = att_sheet.date_from
            to_date = att_sheet.date_to
            ...
            self._cr.execute('''
                SELECT id FROM hr_payslip WHERE employee_id = %s and state = 'draft'	            
                ''', ((self.employee_id.id),))

            select_employee = self._cr.fetchone()
            if select_employee:
                print ('select_employee')
                print (select_employee)

            self._cr.execute('''
                INSERT INTO hr_payslip_worked_days
                (name, payslip_id, sequence, code, number_of_days, number_of_hours, contract_id, create_uid, create_date)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())
    ''', ("Overetime",(select_employee),(30),('OVT'),(att_sheet.no_overtime),(att_sheet.tot_overtime),(contract_id),(self.env.uid),))

            self._cr.execute('''
                INSERT INTO hr_payslip_worked_days
                (name, payslip_id, sequence, code, number_of_days, number_of_hours, contract_id, create_uid, create_date)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())
    ''', ("Absence",(select_employee),(35),('ABS'),-(att_sheet.no_absence),(att_sheet.tot_absence),(contract_id),(self.env.uid),))

            self._cr.execute('''
                INSERT INTO hr_payslip_worked_days
                (name, payslip_id, sequence, code, number_of_days, number_of_hours, contract_id, create_uid, create_date)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())
    ''', ("Late In",(select_employee),(40),('LATE'),-(att_sheet.no_late),(att_sheet.tot_late),(contract_id),(self.env.uid),))

            self._cr.execute('''
                INSERT INTO hr_payslip_worked_days
                (name, payslip_id, sequence, code, number_of_days, number_of_hours, contract_id, create_uid, create_date)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())
    ''', ("Difference Time",(select_employee),(45),('DIFFT'),-(att_sheet.no_difftime),(att_sheet.tot_difftime),(contract_id),(self.env.uid),))
...
#            print(res)
            new_payslip = self.env['hr.payslip'].update(res)
...
class attendance_sheet_line(models.Model):
    ...
    worked_hours = fields.Float("Extra Hours", readonly=True)
